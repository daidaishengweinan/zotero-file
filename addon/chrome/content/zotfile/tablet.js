...
Zotero.ZotFile.Tablet = new (function () {
  ...
  this.addTabletTag = function (att, tag) {
    ...
    // get tag IDs
    var tag_remove =
      tag == this.Tablet.tag ? this.Tablet.tagMod : this.Tablet.tag;
  ...
  this.removeTabletTag = function (att, tag) {
    ...
    if (item.hasTag(tag)) {
      var atts = Zotero.Items.get(item.getAttachments());
      if (!atts.some((att) => att.hasTag(tag))) item.removeTag(tag);
    }
  }.bind(Zotero.ZotFile);
  ...
  this.getSelectedAttachmentsFromTablet = Zotero.Promise.coroutine(
    function* () {
  ...
  this.clearInfo = function (att) {
    try {
      ...
      var content = att
          .getNote()
          .replace(/zotero:\/\//g, "http://zotfile.com/"),
        doc = parser.parseFromString(content, "text/html"),
        p = doc.querySelector("#zotfile-data");
      if (p === null)
        p = doc.querySelector('[title*="lastmod"][title*="projectFolder"]');
  ...
  this.getInfo = function (att, key) {
    try {
      ...
      if (key == "location")
        value = value.replace("[BaseFolder]", this.getPref("tablet.dest_dir"));
      // for location and projectFolder tag: correct window/mac file system
      if (["location", "projectFolder"].includes(key) && Zotero.isWin)
  ...
  this.getTabletStatus = function (att) {
    ...
    return (
      att.isAttachment() &&
      (att.hasTag(this.Tablet.tag) || att.hasTag(this.Tablet.tagMod))
    );
  }.bind(Zotero.ZotFile);
   ...
   */
  this.getTabletStatusModified = Zotero.Promise.coroutine(
    function* (att) {
      if (!this.Tablet.getTabletStatus(att)) return false;
      ...
      var lastmod_tablet = this.Tablet.getInfo(att, "lastmod"),
        lastmod_file =
          Date.parse((yield OS.File.stat(path)).lastModificationDate) + "";
   ...
   * @yield {void}
   */
  this.showTabletFile = Zotero.Promise.coroutine(
    function* () {
      var win = Services.wm.getMostRecentWindow("navigator:browser"),
   ...
   * @yield {void}
   */
  this.openTabletFile = Zotero.Promise.coroutine(
    function* () {
      var win = Services.wm.getMostRecentWindow("navigator:browser"),
        att = win.ZoteroPane.getSelectedItems()[0],
        tablet = this.Tablet.getTabletStatus(att);
   ...
   */
  this.getTabletFilePath = Zotero.Promise.coroutine(
    function* (att, verbose) {
  ...
  this.tabletFileMissing = Zotero.Promise.coroutine(
    function* (att) {
      // foreground mode
      if (this.Tablet.getInfo(att, "mode") == 2) {
  ...
  this.getAttachmentsOnTablet = Zotero.Promise.coroutine(
    function* (subfolder) {
      ...
      var atts = Zotero.Items.get(search_results).filter(
        (item) => item.isAttachment() && !item.isTopLevelItem(),
      );
      // show warning if no information in note
      if (this.getPref("tablet.showwarning"))
        atts
          .filter((att) => this.Tablet.getInfo(att, "mode") === "")
          .forEach((att) =>
            this.infoWindow(
              this.ZFgetString("general.warning"),
              this.ZFgetString("tablet.attachmentNoteMissing") +
                " (" +
                att.key +
                ")",
            ),
      ...
      atts = atts
        ...
        .filter(
          (att) =>
            subfolder === undefined ||
            this.Tablet.getInfo(att, "projectFolder").toLowerCase() ==
  ...
  this.getModifiedAttachmentsOnTablet = Zotero.Promise.coroutine(
    function* (subfolder) {
      var atts = yield this.Tablet.getAttachmentsOnTablet(subfolder);
      atts = yield Zotero.Promise.filter(atts, (att) =>
        this.Tablet.getTabletStatusModified(att),
      );
      return atts;
    }.bind(Zotero.ZotFile),
  );

  this.setTabletFolder = Zotero.Promise.coroutine(
    function* (atts, projectFolder) {
      projectFolder = typeof projectFolder !== "undefined" ? projectFolder : "";
      var tablet_folder = this.Utils.joinPath(
          this.getPref("tablet.dest_dir"),
          projectFolder,
        ),
        tablet_subfolder =
          !this.getPref("import") & this.getPref("tablet.subfolder")
            ? this.getPref("tablet.subfolderFormat")
            : "";
      ...
      var loc =
        projectFolder !== ""
          ? "'..." + projectFolder + "'."
          : this.ZFgetString("tablet.baseFolder");
      var progressWin = this.progressWindow(
        this.ZFgetString("tablet.movedAttachments", [loc]),
      );
      ...
      for (let i = 0; i < atts.length; i++) {
        ...
        try {
          ...
          if (att_mode == 1) yield this.moveFile(path, pathNew);
          // update tablet folder (foreground mode)
          if (att_mode == 2)
            yield this.moveLinkedAttachmentFile(
              att,
              folder,
          ...
          this.Tablet.addInfo(att, {
            lastmod:
              time_saved == time_tablet
                ? Date.parse((yield OS.File.stat(pathNew)).lastModificationDate)
                : time_saved,
            location: pathNew,
  ...
  this.checkSelectedSearch = function () {
    ...
    for (var key in conditions) {
      ...
      if (
        cond.condition == "tag" &&
        [this.Tablet.tag, this.Tablet.tagMod].includes(cond.value)
      )
        return true;
    }
    return false;
  ...
  this.updateModifiedAttachmentsSearch = Zotero.Promise.coroutine(
    function* () {
  ...
  this.restrictTabletSearch = Zotero.Promise.coroutine(
    function* (which) {
      var win = Services.wm.getMostRecentWindow("navigator:browser"),
   ...
   */
  this.sendAttachmentToTablet = Zotero.Promise.coroutine(
    function* (att, project_folder, verbose) {
      ...
      var tablet_mode = this.getPref("tablet.mode"),
        ...
        tablet_dest = this.Utils.joinPath(
          this.getPref("tablet.dest_dir"),
          project_folder,
        ),
        tablet_subfolder =
      ...
      if (tablet_mode == 1) {
        ...
        if (tablet_status) {
          ...
          if (path_tablet) {
            ...
            this.infoWindow(
              "ZotFile Warning",
              "File on tablet not found. Zotfile is creating a new copy on tablet.",
            );
            path = yield this.copyFile(path, folder, att.attachmentFilename);
          }
        }
      ...
      if (tablet_mode == 2) {
        att = yield this.renameAttachment(
          att,
          false,
          tablet_rename,
          tablet_dest,
      ...
      if (!tablet_status) {
        ...
        if (this.getPref("tablet.tagParentPush"))
          item.addTag(this.getPref("tablet.tagParentPush_tag"));
        yield item.saveTx();
      }
      yield att.saveTx();
      // notification
      if (verbose)
        this.messages_report.push("'" + OS.Path.basename(path) + "'");
      return att;
    }.bind(Zotero.ZotFile),
  );
   ...
   * @yield {void}
   */
  this.sendSelectedAttachmentsToTablet = Zotero.Promise.coroutine(
    function* (idx_subfolder) {
      ...
      // confirm
      var atts_tablet = yield Zotero.Promise.filter(
          atts,
          (att) =>
            this.Tablet.getTabletStatus(att) &&
            this.Tablet.getTabletFilePath(att, false),
        ),
      ...
      if (
        ...
        if (!confirm(this.ZFgetString("tablet.sendAttachments", [atts.length])))
          return;
      if (!pref_repush && atts_tablet.length > 0)
        pref_repush = confirm(
          this.ZFgetString("tablet.replaceAttachAlready", [atts_tablet.length]),
        );
      // show infoWindow
      var progressWin = this.progressWindow(
          this.ZFgetString("tablet.AttsMoved"),
        ),
      ...
      for (let i = 0; i < atts.length; i++) {
        ...
        if (this.Tablet.getTabletStatus(att) && path_tablet && !pref_repush)
          continue;
        ...
        if (
          this.Tablet.getTabletStatus(att) &&
          (att_mode != this.getPref("tablet.mode") || !path_tablet)
        )
          att = yield this.Tablet.getAttachmentFromTablet(att, true);
        // send to tablet
        att = yield this.Tablet.sendAttachmentToTablet(
          att,
          project_folder,
          false,
        );
  ...
  this.updateSelectedTabletAttachments = Zotero.Promise.coroutine(
    function* () {
      // get selected attachments, filter for tablet
      var atts = Zotero.Items.get(this.getSelectedAttachments())
        .filter(this.Tablet.getTabletStatus)
      ...
      // show notification
      var progressWin = this.progressWindow(
          "ZotFile: Update Modification Time",
        ),
        description = atts.length == 0;
      ...
      for (let i = 0; i < atts.length; i++) {
        ...
        if (att_mode == 2) {
          let lastModificationDate = Date.parse(
            (yield OS.File.stat(path)).lastModificationDate,
          );
  ...
  this.getLastModifiedTabletFile = Zotero.Promise.coroutine(
    function* (att) {
      // foreground mode
      if (this.Tablet.getInfo(att, "mode") == 2)
        return yield att.getFilePathAsync();
      ...
      var time_tablet = path_tablet
        ...
        time_zotero = path_zotero
          ...
          : 0;
      if (time_tablet == 0 && time_zotero == 0) return false;
      // set options
  ...
  this.getAttachmentFromTablet = Zotero.Promise.coroutine(
    function* (att, fake_remove) {
      var item = Zotero.Items.get(att.parentItemID),
        tablet_status = 1,
        item_pulled = false,
        att_deleted = false,
      ...
      if (path_tablet === false) {
        ...
        if (!fake_remove)
          this.infoWindow(
            ...
            'The tablet file "' +
              att.attachmentFilename +
              '" was manually moved and does not exist.',
          );
        return att;
      }
      ...
      if (att_mode == 1 && (time_tablet != 0 || time_zotero != 0)) {
        ...
        if (!this.getPref("tablet.storeCopyOfFile")) {
          ...
          if (tablet_status == 1) {
            tablet_status = this.promptUser(
              this.ZFgetString("tablet.fileConflict", [att.attachmentFilename]),
        ...
        if (this.getPref("tablet.storeCopyOfFile") && tablet_status != 2) {
          var filename = this.Utils.addSuffix(
            OS.Path.basename(path_zotero),
            this.getPref("tablet.storeCopyOfFile_suffix"),
          );
          //add linked attachment
          if (item.library.libraryType == "user" && !this.getPref("import")) {
            let path_copied = OS.Path.join(
              OS.Path.dirname(path_zotero),
              filename,
            );
      ...
      if (att_mode == 2) {
        ...
        att = yield this.renameAttachment(
          att,
          this.getPref("import"),
          this.getPref("tablet.rename"),
          this.getPref("dest_dir"),
          subfolder,
          false,
        );
        // finish up
      ...
      if (
        attSubfolder != "" &&
        tablet_folder != OS.Path.normalize(this.getPref("tablet.dest_dir"))
      )
      ...
      if (item_pulled && !fake_remove) {
        ...
        // extract annotations from attachment and add note
        if (
          this.getPref("pdfExtraction.Pull") &&
          tablet_status != 2 &&
          !this.getPref("tablet.storeCopyOfFile")
        )
        ...
        var message =
          ...
          att.attachmentFilename +
          "' " +
          (att_deleted ? this.ZFgetString("tablet.attsDel") : "");
