...
Zotero.ZotFile.Utils = new (function () {
  ...
  this.removeFromArray = function (arr) {
    var what,
      a = Array.prototype.slice.call(arguments).splice(1),
      L = a.length,
      ax;
    while (L && arr.length) {
      what = a[--L];
  ...
  this.getPDFReader = function () {
    if (!Zotero.isWin)
      throw "Zotero.ZotFile.Utils.getPDFReader(): Function only works on windows platforms.";
    var wrk = Components.classes[
  ...
  this.removeItemTag = function (item, tag) {
    if (item.isRegularItem() && item.hasTag(tag)) {
      if (
        !Zotero.Items.get(item.getAttachments()).some(function (att) {
          return att.hasTag(tag);
        })
      )
        item.removeTag(tag);
    }
  ...
  this.parseHTML = function (html) {
    ...
    if (PARSER_UTILS in Components.classes) {
      var parser = Components.classes[PARSER_UTILS].getService(
