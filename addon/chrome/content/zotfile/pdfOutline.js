...
Zotero.ZotFile.pdfOutline = new (function () {
  ...
  this.getOutline = Zotero.Promise.coroutine(function* (attIDs, verbose) {
    ...
    this.atts = yield Zotero.Promise.filter(this.atts, (att) =>
      att.fileExists(),
    );
    ...
    if (Zotero.isFx36) {
      Zotero.ZotFile.infoWindow(
        ...
        Zotero.ZotFile.ZFgetString("extraction.outdatedFirefox"),
      );
      return;
    }
    // progresswindow
    ...
    for (var i = 0; i < this.atts.length; i++) {
      this.itemProgress.push(
        new this.progressWin.ItemProgress(
          icon_pdf,
          this.atts[i].getField("title"),
        ),
      );
    }
    this.progressWin.addDescription(
      "Zotfile can only extract the TOC for PDFs that have a TOC.",
    );
  ...
  this.createOutline = function (att, outline, itemProgress) {
    ...
    var create_toc = function (entry) {
      ...
      if (!firstElement)
        li.setAttribute(
          "style",
          entry.items.length > 0 ? "padding-top:8px" : "padding-top:4px",
        );
      firstElement = false;
      a.setAttribute(
        "href",
        zz.Utils.str_format(href, { lib: lib, key: key, page: entry.page + 1 }),
      );
      a.textContent = Zotero.Utilities.htmlSpecialChars(entry.title);
      a.textContent = a.textContent.replace(/&apos;/g, "'");
      if (entry.page !== undefined) li.appendChild(a);
      if (entry.page !== undefined && entry.items.length > 0) lvl++;
      // add subitems
      if (entry.items.length > 0 && lvl <= zz.getPref("pdfOutline.tocDepth")) {
        var ul = win.document.createElementNS(zz.xhtml, "ul");
        ul.setAttribute(
          "style",
          zz.Utils.str_format(style, { padding: 12 * (lvl - 1) }),
        );
