...
Zotero.ZotFile.pdfAnnotations = new (function () {
  ...
  this.PDF_EXTRACT_URL =
    "chrome://zoterofile/content/zotfile/pdfextract/extract.html";
  ...
  this.popplerExtractorSetPath = function () {
    ...
    // extractor path
    this.popplerExtractorPath = OS.Path.join(
      Zotero.DataDirectory.dir,
      "ExtractPDFAnnotations",
      this.popplerExtractorFileName,
    );
  };
  ...
  this.getAnnotations = Zotero.Promise.coroutine(function* (attIDs) {
    ...
    // progress window
    this.progressWin = Zotero.ZotFile.progressWindow(
      "Zotfile: Extracting Annotations...",
    );
    if (atts.length == 0) {
      this.progressWin.addDescription(
        Zotero.ZotFile.ZFgetString("general.warning.skippedAtt.msg"),
      );
      this.progressWin.startCloseTimer(
        Zotero.ZotFile.getPref("info_window_duration"),
      );
      return;
    }
    // extraction settings
    var settings_pdfjs = Zotero.ZotFile.getPref("pdfExtraction.UsePDFJS"),
      settings_both = Zotero.ZotFile.getPref(
        "pdfExtraction.UsePDFJSandPoppler",
      ),
    ...
    for (let i = 0; i < atts.length; i++) {
      ...
      let att = atts[i],
        ...
        path = tabletStatus
          ? yield Zotero.ZotFile.Tablet.getLastModifiedTabletFile(att)
          : yield att.getFilePathAsync();
      ...
      this.pdfAttachmentsForExtraction.push({
        ...
        item: item,
        itemProgress: new this.progressWin.ItemProgress(
          icon_pdf,
  ...
  this.popplerExtractor = Zotero.Promise.coroutine(function* (setProgress) {
    ...
    for (let i = 0; i < this.pdfAttachmentsForExtraction.length; i++) {
      ...
      yield Zotero.Utilities.Internal.exec(this.popplerExtractorPath, [
        path,
        outputFile,
      ]);
      // get annotations from file and create note
      let annotations = yield this.popplerExtractorGetAnnotationsFromFile(
        outputFile,
      );
  ...
  this.popplerExtractorGetAnnotationsFromFile = Zotero.Promise.coroutine(
    function* (path) {
      var annotations = [];
      // check whether file exists
      if (!(yield OS.File.exists(path))) {
        Zotero.ZotFile.messages_error.push(
          Zotero.ZotFile.ZFgetString("extraction.failedNoFile"),
        );
        return [];
      }
      // read file
  ...
  this.createNote = Zotero.Promise.coroutine(
    function* (annotations, item, att, method) {
      // get note content
  ...
  this.getNoteContent = function (annotations, item, att, method) {
    var lib = att.library.libraryType == "user" ? 0 : att.libraryID,
      groupID =
        lib != 0
          ? Zotero.Groups.getGroupIDFromLibraryID(att.libraryID)
          : undefined,
      format_uri = "zotero://open-pdf/library/items/%(key)?page=%(page)",
      format_uri_group =
        "zotero://open-pdf/groups/%(groupID)/items/%(key)?page=%(page)",
      ...
      format_highlight = this.getPref(
        "pdfExtraction.formatAnnotationHighlight",
      ),
      format_underline = this.getPref(
        "pdfExtraction.formatAnnotationUnderline",
      ),
      settings_colors = JSON.parse(
        this.getPref("pdfExtraction.colorCategories"),
      ),
      setting_color_notes = this.getPref("pdfExtraction.colorNotes"),
      setting_aggregate_color_highlights = this.getPref(
    ...
    for (var i = 0; i < annotations.length; i++) {
      ...
      if (this.getPref("pdfExtraction.NoteTruePage")) {
        try {
          ...
          if (itemPages) {
            var page_parsed =
              typeof itemPages == "string"
                ? parseInt(itemPages.split("-")[0], 10)
                : itemPages;
      ...
      var link = '<a href="' + uri + '">' + cite + page + "</a>",
        color =
          "color" in anno
            ? "rgb(" + anno.color.join(",") + ")"
            : "rgb(255,255,255)",
        color_category = this.pdfAnnotations.getColorCategory(
          anno.color[0],
          anno.color[1],
          anno.color[2],
        ),
      ...
      if (anno.markup && anno.markup != "") {
        var format_markup =
          anno.subtype == "Highlight" ? format_highlight : format_underline;
        ...
        if (!setting_color_notes) note += markup_formated;
        else {
      ...
      if (
        ...
        anno.content != "" &&
        (!anno.markup ||
          this.Utils.strDistance(anno.content, anno.markup) > 0.15)
      ) {
        ...
        if (!setting_color_notes) note += content_formated;
        else {
          if (!(color_category in note)) {
            note[color_category] = this.Utils.str_format(format_title_color, {
              title: str_title,
  ...
  this.cancellationListener = function (keyEvent) {
    if (keyEvent.keyCode == KeyboardEvent.DOM_VK_ESCAPE) {
      ...
      keyEvent.currentTarget.removeEventListener(
        "keypress",
        zzpa.cancellationListener,
      );
    }
  };
  ...
  this.extractionComplete = function (annotations, item, att) {
    ...
    if (annotations.length > 0)
      this.createNote(annotations, item, att, "pdf.js");
    ...
    if (this.pdfAttachmentsForExtraction.length > 0) {
      ...
      if (this.errorExtractingAnnotations) {
        Zotero.ZotFile.infoWindow(
          Zotero.ZotFile.ZFgetString("general.report"),
          Zotero.ZotFile.ZFgetString("extraction.pdfjsFailed"),
        );
      }
      this.errorExtractingAnnotations = false;
