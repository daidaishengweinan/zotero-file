...
Zotero.ZotFile.notifierCallback = new (function () {
   ...
   * @return {void}
   */
  this.notify = Zotero.Promise.coroutine(
    function* (event, type, ids, extraData) {
      console.log(event, type, ids, this.getPref("automatic_renaming"));
      // automatic renaming
      if (
        type == "item" &&
        event == "add" &&
        this.getPref("automatic_renaming") != 1
      ) {
      ...
      if (
        type == "item" &&
        event == "add" &&
        this.getPref("pdfOutline.getToc")
      ) {
        let atts = Zotero.Items.get(ids)
        ...
        if (atts.length > 0)
          setTimeout(
            () => this.pdfOutline.getOutline(atts.map((att) => att.id)),
            100,
          );
      }
    }.bind(Zotero.ZotFile),
   ...
   * @return {void}
   */
  var rename = Zotero.Promise.coroutine(
    function* (attachments) {
      console.log("rename is called", attachments);
      ...
      for (var i = 0; i < attachments.length; i++) {
        ...
        if (
          this.excludeAutorenameKeys.includes(att.key) ||
          this.excludeAutorenameKeys.includes(item.key)
        ) {
        ...
        if (filename.indexOf(this.getFilename(item, filename)) === 0) {
          console.log("skip 2");
          continue;
        }
        ...
        var message = {
          ...
          txt: "(click here to move and rename)",
        ...
        if (auto_rename == 3) {
          ...
          if (item_atts.length > 1)
            this.infoWindow("ZotFile: New attachment", message, duration, () =>
              on_confirm(att),
            );
        }
        // always rename
  ...
  var on_confirm = Zotero.Promise.coroutine(
    function* (att) {
      ...
      var progress_win = this.progressWindow(
        "ZotFile: New attachment file renamed",
      );
      var progress = new progress_win.ItemProgress(
        att.getImageSrc(),
        att.getField("title"),
      );
