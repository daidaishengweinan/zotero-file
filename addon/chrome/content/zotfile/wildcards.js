...
Zotero.ZotFile.Wildcards = new (function () {
  ...
  function truncateTitle(title) {
    ...
    if (title.length > Zotero.ZotFile.getPref("max_titlelength")) {
      ...
      if (
        Zotero.ZotFile.getPref("truncate_smart") &&
        title.search(" ") != -1 &&
        before_trunc_char.search(/[a-zA-Z0-9]/ != -1)
      ) {
  ...
  function findOuterPairs(rule, open, close) {
      ...
      if (rule[i] === open) {
        ...
        if (matching.length === 0) {
          Zotero.ZotFile.messages_error.push(
            Zotero.ZotFile.ZFgetString("renaming.errorFormat.closing", [
              close,
              i,
            ]),
          );
    ...
    if (matching.length > 0) {
      Zotero.ZotFile.messages_error.push(
        Zotero.ZotFile.ZFgetString("renaming.errorFormat.opening", [
          open,
          matching[0],
        ]),
      );
  ...
  function formatAuthors(item) {
    ...
    for (var i = 0; i < creators.length; ++i) {
      if (creatorTypeIDs.indexOf(creators[i].creatorTypeID) === -1)
        numauthors = numauthors - 1;
    }
    var max_authors = Zotero.ZotFile.getPref("truncate_authors")
      ? Zotero.ZotFile.getPref("max_authors")
      : 500;
    ...
    var lastAuthor = "",
      lastAuthor_lastf = "",
      lastAuthor_initials = "",
      lastAuthor_lastInitial = "";
    if (creators.length > 0) {
      lastAuthor = creators[creators.length - 1].lastName;
      lastAuthor_lastf =
        creators[creators.length - 1].lastName +
        creators[creators.length - 1].firstName.substr(0, 1).toUpperCase();
      lastAuthor_initials =
        creators[creators.length - 1].firstName.substr(0, 1).toUpperCase() +
        creators[creators.length - 1].lastName.substr(0, 1).toUpperCase();
      lastAuthor_lastInitial = creators[creators.length - 1].lastName
        .substr(0, 1)
        .toUpperCase();
    }
    ...
    for (var i = 0; i < creators.length; ++i) {
      if (editorType.indexOf(creators[i].creatorTypeID) === -1)
    ...
    for (i = 0; i < creators.length; ++i) {
      if (
        j < numeditors &&
        editorType.indexOf(creators[i].creatorTypeID) != -1
      ) {
  ...
  function wildcardTable(item) {
    ...
    var itemtypeWildcard = function (item, map) {
      ...
      if (typeof property == "string")
        value =
          property in addFields
            ? addFields[property]
            : item.getField(property, false, true);
      if (typeof property == "object") value = regexWildcard(item, property);
      return value;
    };
    var regexWildcard = function (item, w) {
      ...
      if (typeof field == "string")
        output =
          field in addFields
            ? addFields[field]
            : item.getField(field, false, true);
      if (typeof field == "object") output = itemtypeWildcard(item, field);
      // operations
      if (operations !== undefined) {
        for (var i = 0; i < operations.length; ++i) {
          ...
          // simple functions
          if (obj.function == "toLowerCase") output = output.toLowerCase();
          if (obj.function == "toUpperCase") output = output.toUpperCase();
          if (obj.function == "trim") output = output.trim();
          if (obj.function == "truncateTitle") output = truncateTitle(output);
        }
      }
      // return
      return output;
    ...
    for (var key in wildcards_user) {
      wildcards[key] = wildcards_user[key];
    }
  ...
  function fillRule(rule, table, offset) {
    ...
    for (var i = 0; i < bars.length; ++i) {
      ...
      if (pos - 1 < last || pos === 0) {
        Zotero.ZotFile.messages_error.push(
          Zotero.ZotFile.ZFgetString("renaming.errorFormat.left", [
            offset + bars[i],
          ]),
        );
      }
      ...
      if (wildcards[pos] > bars[i + 1] || pos === wildcards.length) {
        Zotero.ZotFile.messages_error.push(
          Zotero.ZotFile.ZFgetString("renaming.errorFormat.right", [
            offset + bars[i],
          ]),
        );
      }
    ...
    for (var j = last + 1; j < wildcards.length; ++j) {
      ...
      if (lookup && Array.isArray(lookup)) {
        var getCollectionPathFromTable = function () {
          ...
          var idx = collectionPaths.indexOf(ZoteroPane.getSelectedCollection().name)
  ...
  this.replaceWildcard = function (item, rule, table, offset) {
    ...
    for (var i = 0; i < conditional.length; ++i) {
      res = fillRule(
        rule.substring(last + 1, conditional[i].start),
        table,
        last + 1,
      );
      ...
      name.push(res.str);
      name.push(
        this.replaceWildcard(
          item,
          rule.substring(conditional[i].start + 1, conditional[i].end),
          table,
          conditional[i].start + 1,
        ),
      );
