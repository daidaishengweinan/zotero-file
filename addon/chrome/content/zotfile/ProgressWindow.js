...
Zotero.ZotFile.ProgressWindow = function (_window) {
  ...
  this.show = function show() {
    if (_windowLoading || _windowLoaded) {
   ...
   */
  this.changeHeadline = _deferUntilWindowLoad(
    function changeHeadline(text, icon, postText) {
  ...
  this.addDescription = _deferUntilWindowLoad(function addDescription(text) {
    ...
    _progressWindow.document
      .getElementById("zotero-progress-text-box")
      .appendChild(newHB);
  ...
  this.ItemProgress = _deferUntilWindowLoad(
    function (iconSrc, title, parentItemProgress) {
      ...
      var container = _progressWindow.document.getElementById(
   ...
   */
  this.ItemProgress.prototype.setProgress = _deferUntilWindowLoad(
    function (percent) {
      if (percent != 0 && percent != 100) {
        var nArcs = 20;
        // Indication of partial progress, so we will use the circular indicator
        this._image.style.backgroundImage =
          "url('chrome://zotero/skin/progress_arcs.png')";
        this._image.style.backgroundPosition =
          "-" + Math.round((percent / 100) * nArcs) * 16 + "px 0";
        this._hbox.style.opacity = percent / 200 + 0.5;
   ...
   * @param {Integer} percent A percentage from 0 to 100.
   */
  this.ItemProgress.prototype.setIcon = _deferUntilWindowLoad(
    function (iconSrc) {
  ...
  this.ItemProgress.prototype.setTitle = _deferUntilWindowLoad(
    function (title) {
      this._itemText.textContent = title;
    },
  );

  this.ItemProgress.prototype.complete = _deferUntilWindowLoad(
    function (title, icon) {
  ...
  function _onWindowLoaded() {
    ...
    for (var i = 0; i < _deferredUntilWindowLoad.length; i++) {
      _deferredUntilWindowLoad[i].apply(
        _deferredUntilWindowLoadThis[i],
  ...
  function _onMouseOut(e) {
    ...
    if (
      ...
      e.screenX <= this.screenX + this.outerWidth &&
      e.screenY >= top &&
      e.screenY <= top + this.outerHeight
    ) {
      return;
    }
  ...
  function _deferUntilWindowLoad(fn) {
    return function () {
      ...
      if (_windowLoaded) {
        ...
        _deferredUntilWindowLoadArgs.push(
