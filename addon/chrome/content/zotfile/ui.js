...
Zotero.ZotFile.UI = new (function () {
  ...
  this.showCollectionMenu = function () {
    // ZoteroPane object
    var doc =
      Services.wm.getMostRecentWindow("navigator:browser").ZoteroPane.document;
    // check regular item or attachment selected & custom subfolders
    var collection_menu =
      this.getPref("tablet") &&
      this.Tablet.checkSelectedSearch() &&
      this.getPref("tablet.projectFolders") == 2;
    // show or hide zotfile menu items
    doc.getElementById("id-zotfile-collection-separator").hidden =
      !collection_menu;
    doc.getElementById("id-zotfile-collection-showall").hidden =
      !collection_menu;
    doc.getElementById("id-zotfile-collection-restrict").hidden =
      !collection_menu;
  }.bind(Zotero.ZotFile);

  this.buildZotFileCollectionMenu = function () {
    var win = Services.wm.getMostRecentWindow("navigator:browser"),
      nodes = win.ZoteroPane.document.getElementById(
        "id-zotfile-collection-menu",
      ).childNodes;
    ...
    subfolders.forEach(function (folder, i) {
      ...
      nodes[i + 3].setAttribute(
        "tooltiptext",
        this.ZFgetString("menu.collection.tooltip", [folder.path]),
      );
      // show menu item
  ...
  this.showMenu = function () {
    ...
    pane.document.getElementById("id-zotfile-manage-attachments").hidden =
      !show_menu;
  }.bind(Zotero.ZotFile);
  ...
  this.buildZotFileMenu = Zotero.Promise.coroutine(function* () {
    ...
    var disable = [m.tablet, m.warning1, m.warning2, m.warning3],
      show = [];
    // check selected items
    var tags_tablet = [this.Tablet.tag, this.Tablet.tagMod];
    var atts = items
      .map((item) =>
        item.isRegularItem() ? Zotero.Items.get(item.getAttachments()) : item,
      )
      .reduce((a, b) => a.concat(b), []);
    var group_library = items[0].libraryID != 1,
      menu_item = items.some(
        (item) => item.isRegularItem() || item.isAttachment(),
      ),
    ...
    var valid_destination = yield OS.File.exists(
      this.getPref("tablet.dest_dir"),
    );
    // warnings
    if (!menu_item) {
      show.push(m.warning1);
      menu.childNodes[m.warning1].setAttribute(
        "label",
        this.ZFgetString("menu.noItemSelected"),
      );
    }
    // item menu
    if (menu_item) {
      ...
      if (!menu_att) {
        ...
        menu.childNodes[m.warning2].setAttribute(
          "label",
          this.ZFgetString("menu.itemHasNoAtts"),
        );
      }
      // add 'Extract annotations'
      if (
        !Zotero.ZotFile.isZotero6OrLater &&
        this.getPref("pdfExtraction.MenuItem")
      )
        show.push(m.extractanno);
      ...
      if (this.getPref("tablet") && menu_att) {
        ...
        if (!valid_destination) {
          ...
          menu.childNodes[m.warning2].setAttribute(
            "label",
            this.ZFgetString("menu.invalidTabletLocation"),
          );
        }
        if (group_library) {
          show.push(m.warning2);
          menu.childNodes[m.warning2].setAttribute(
            "label",
            this.ZFgetString("menu.itemIsInGroupLibrary"),
          );
        }
        if (valid_destination && !group_library) {
          show.push(m.push2reader, m.pullreader);
          // set tooltip for base folder
          menu.childNodes[m.push2reader].setAttribute(
            "tooltiptext",
            this.ZFgetString("menu.sendAttToBaseFolder", [
              this.getPref("tablet.dest_dir"),
            ]),
          );
          ...
          if (
            this.Tablet.checkSelectedSearch() ||
            this.getPref("tablet.updateAlwaysShow")
          ) {
          ...
          if (this.getPref("tablet.projectFolders") == 1) {
            ...
            if (item.isAttachment())
              if (item.parentItemID) item = Zotero.Items.get(item.parentItemID);
            ...
            if (folders.length) {
              ...
              for (i = 0; i < folders.length; i++) {
                ...
                menu.childNodes[m.subfolders[i]].setAttribute(
                  "label",
                  folders[i],
                );
                menu.childNodes[m.subfolders[i]].setAttribute(
                  "tooltiptext",
                  this.ZFgetString("menu.sendAttToSubfolder", [folders[i]]),
                );
          ...
          if (!projects_set && this.getPref("tablet.projectFolders") != 0) {
            ...
            if (this.getPref("tablet.projectFolders") == 1)
              warning = this.ZFgetString("menu.itemIsInNoCollection");
            if (this.getPref("tablet.projectFolders") == 2)
              warning = this.ZFgetString("menu.noSubfoldersDefined");
  ...
  this.buildTabletMenu = function () {
    ...
    var menu_items = [
      {
        ...
        tooltiptext: "",
        command: function (e) {
          Zotero.ZotFile.Tablet.openTabletFile();
        },
      ...
      },
      {
        ...
        tooltiptext: "",
        command: function (e) {
          Zotero.ZotFile.Tablet.showTabletFile();
        },
        hidden: tablet ? "false" : "true",
      },
      {
    ...
    for (i in menu_items) {
      ...
      for (key in Object.keys(item))
        ...
        else menuitem.addEventListener(keys[key], item[keys[key]]);
    ...
    if (this.getPref("tablet.projectFolders") == 2) {
      ...
      menuitem.setAttribute(
        "style",
        "font-size: 80%; background: none; -moz-appearance: none;",
      );
      menupopup.appendChild(menuitem);
      // add subfolders
      var subfolders = JSON.parse(this.getPref("tablet.subfolders"));
