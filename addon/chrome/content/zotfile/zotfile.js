...
Zotero.ZotFile = new (function () {
  ...
  this.versionChanges = function (version) {
    ...
    if (this.getPref("version") === "" || show_changelog.includes(version)) {
      ...
      if (Zotero.isStandalone)
        this.futureRun(() =>
          ZoteroPane_Local.loadURI(Zotero.ZotFile.changelogURL),
        );
    }
  ...
  this.init = async function () {
    ...
    if (!_initialized) {
      ...
      if (Zotero.isStandalone && this.getPref("source_dir_ff"))
      ...
      this.futureRun(
        function () {
          ...
          this.pdfAnnotations.popplerExtractorSupported =
            this.pdfAnnotations.popplerSupportedPlatforms.includes(
              Zotero.platform,
            );
          ...
          if (this.pdfAnnotations.popplerExtractorSupported) {
            ...
            OS.File.exists(this.pdfAnnotations.popplerExtractorPath).then(
              (poppler) => {
          ...
          if (!this.pdfAnnotations.popplerExtractorSupported)
            this.setPref("pdfExtraction.UsePDFJS", true);
        }.bind(this),
      );
    }
    // Register callbacks in Zotero as item observers
    window.alert("zotfile: Zotero.Notifier.registerObserver");
    ...
    Components.classes["@mozilla.org/moz/jssubscript-loader;1"]
      ...
      .loadSubScript(
        "chrome://zotfile/content/ProgressWindow.js",
        Zotero.ZotFile,
      );
  ...
  this.zotero7transition = Zotero.Promise.coroutine(function* (startup) {
    ...
    if (atts.length == 0) {
      ...
      if (!this.getPref("tablet")) {
        let message =
          "You are ready to upgrade to Zotero 7!\n\n" +
      ...
      if (confirm(message)) this.setPref("tablet", false);
    ...
    // missing tablet file
    var atts_valid = yield Zotero.Promise.filter(atts, (att) =>
      OS.File.exists(this.Tablet.getInfo(att, "location")),
    );
    var atts_missing = yield Zotero.Promise.filter(
      atts,
      this.Tablet.tabletFileMissing,
    );
    ...
    if (prompt == 2) {
      ...
      for (var i = 0; i < atts_valid.length; i++) {
        ...
        if (time_tablet > time_saved && time_zotero <= time_saved)
          tablet_status = 0;
        if (time_tablet <= time_saved && time_zotero <= time_saved)
          tablet_status = 2;
        if (time_tablet <= time_saved && time_zotero > time_saved)
        ...
        if (tablet_status == 0) yield OS.File.move(path_tablet, path_zotero);

        // tablet file not modified
        if (tablet_status == 2) yield OS.File.remove(path_tablet);
      ...
      if (both_modified > 0)
        message =
          ...
          "\n\nFor " +
          both_modified +
          " attachments, both the file in Zotero and on the tablet were modified. In these cases, ZotFile created a linked attachment for the tablet file. The linked attachments have the tag 'zotfile_linked_tablet_attachment'.";
      alert(message);
    ...
    if (prompt == 1) {
      var line = new pw.ItemProgress(
        null,
        "Creating linked attachments for tablet files...",
      );
      line.setProgress(0);
      for (var i = 0; i < atts_valid.length; i++) {
        let att = atts_valid[i],
  ...
  this.ZFgetString = function (name, params) {
    ...
    try {
      if (params !== undefined) {
        ...
        l10n = this.stringsBundle.formatStringFromName(
          name,
          params,
          params.length,
        );
  ...
  this.openPreferenceWindow = function (paneID, action) {
    ...
    window.openDialog(
      "chrome://zotfile/content/options.xul",
      "zotfile-options",
  ...
  this.openSubfolderWindow = function (paneID, action) {
    ...
    var prefWindow = window.openDialog(
      "chrome://zotfile/content/options-projects.xul",
  ...
  this.chooseDirectory = async function () {
    ...
    if (Zotero.platformMajorVersion >= 60) {
      ...
      fp.init(
        win,
        Zotero.getString("dataDir.selectDir"),
  ...
  this.showReportMessages = function (title) {
    if (this.messages_report.length > 0)
      this.infoWindow(title, { lines: this.messages_report });
    this.messages_report = [];
  };
  ...
  this.showWarningMessages = function (title, txt) {
    ...
    if (this.messages_warning.length > 0)
      this.infoWindow(title, { lines: this.messages_warning, txt: txt });
    this.messages_warning = [];
  };
  ...
  this.handleErrors = function (error_message) {
    ...
    (on_click = null), (duration = this.getPref("info_window_duration"));
    // fatal errors
    ...
    if (errors.lines.length > 0) {
      ...
      // show errors
      this.infoWindow(
        this.ZFgetString("general.error"),
        errors,
        duration,
        on_click,
      );
  ...
  this.infoWindow = function (main, message, time, callback) {
    ...
    time =
      typeof time !== "undefined" ? time : this.getPref("info_window_duration");
    // show window
    ...
    } else pw.addDescription(message);
  ...
  this.addUserInput = function (filename, original_filename) {
    ...
    var filesuffix = prompt(
      this.ZFgetString("renaming.addUserInput.prompt", [
        original_filename,
        filename,
      ]),
      default_str,
  ...
  this.checkFileType = function (att) {
    ...
    var pos = att.attachmentFilename.lastIndexOf("."),
      ...
      regex = Zotero.ZotFile.getPref("filetypes")
        .toLowerCase()
        .replace(/,/gi, "|");
    // return value
  ...
  this.getFilename = function (item, name, format) {
    ...
    var filename = "",
      ...
      format_default =
        item_type == Zotero.ItemTypes.getID("patent")
    ...
    if (!this.getPref("useZoteroToRename")) {
      ...
      if (this.getPref("replace_blanks"))
  ...
  this.formatSubfolder = function (item, format) {
    ...
    subfolder = OS.Path.split(subfolder)
      .components.map((s) => (s == "" ? "undefined" : s))
   ...
   */
  this.moveLinkedAttachmentFile = Zotero.Promise.coroutine(
    function* (att, location, filename, overwrite) {
      if (
        !att.isAttachment() ||
      ...
      if (!origPath) {
        Zotero.debug(
          "Attachment file not found in moveLinkedAttachmentFile()",
          2,
        );
        return false;
      }
   ...
   */
  this.moveFile = Zotero.Promise.coroutine(
    function* (sourcePath, target, filename) {
      // check arguments
      if (!(yield OS.File.exists(sourcePath)))
        throw "Zotero.ZotFile.moveFile(): 'file' does not exists.";
      // if target is '/' or '\', use file's parent directory
      if (target.trim() == this.folderSep) target = sourceDir;
      ...
      while (yield OS.File.exists(destPath)) {
        ...
        if (k > 999)
          throw (
            "'Zotero.ZotFile.moveFile()': '" + filename + "' already exists."
          );
      }
      // create subfolder
      ...
      // move file to new location
      yield OS.File.move(sourcePath, destPath).catch(
        OS.File.Error,
        function (e) {
   ...
   */
  this.copyFile = Zotero.Promise.coroutine(
    function* (sourcePath, target, filename) {
      // check arguments
      if (!(yield OS.File.exists(sourcePath)))
        throw "Zotero.ZotFile.copyFile(): 'file' does not exists.";
      ...
      while (yield OS.File.exists(destPath)) {
        ...
        if (k > 999)
          throw (
            "'Zotero.ZotFile.copyFile()': '" + filename + "' already exists."
          );
      }
      // create subfolder
      if (!(yield OS.File.exists(destDir))) {
  ...
  this.removeFile = function (file) {
    ...
    try {
      ...
      if (file.isDirectory())
        this.infoWindow(
          this.ZFgetString("general.report"),
          this.ZFgetString("file.removeFolderFailed"),
        );
    }
  };
  ...
  this.removeEmptyFolders = Zotero.Promise.coroutine(function* (folder) {
    ...
    if (
      !folders_zotfile
        .map((dir) => folder.path.startsWith(dir))
        .some((x) => x === true)
    )
      return;
  ...
  this.showFolder = function (folderFile) {
    ...
    try {
      ...
      window.openDialog(
        "chrome://zotero/content/selectableAlert.xul",
        "zotero-reveal-window",
        "chrome",
        io,
      );
    }
  ...
  this.getFilesInFolder = function (path) {
    ...
    if (!dir.isDirectory())
      throw (
        "Zotero.ZotFile.getSortedFilesInDirectory(): '" +
        path +
        "' is not directory."
      );
    var pref_filetypes = this.getPref("useFileTypes"),
      filetypes = this.getPref("filetypes")
        .split(",")
    ...
    while (files.hasMoreElements()) {
      ...
      if (
        file.isDirectory() ||
        file.isHidden() ||
        (pref_filetypes && !filetypes.includes(filetype))
      )
        continue;
      // add file to array
      entries.push(file);
  ...
  this.getFFDownloadFolder = function () {
    ...
    try {
      if (this.ffPrefs.getBoolPref("useDownloadDir")) {
        var download_manager = Components.classes[
          "@mozilla.org/download-manager;1"
        ].getService(Components.interfaces.nsIDownloadManager);
        path = download_manager.userDownloadsDirectory.path;
      }
      if (
        !this.ffPrefs.getBoolPref("useDownloadDir") &&
        this.ffPrefs.prefHasUserValue("lastDir")
      ) {
  ...
  this.getSourceDir = Zotero.Promise.coroutine(function* (message) {
    var source_dir = this.getPref("source_dir_ff")
      ? this.getFFDownloadFolder()
      : this.getPref("source_dir");
    // valid source directory?
    if (source_dir == "" || !(yield OS.File.exists(source_dir))) {
      if (message)
        this.infoWindow(
          this.ZFgetString("general.error"),
          this.ZFgetString("file.invalidSourceFolder"),
        );
      return undefined;
    }
    return source_dir;
  ...
  this.attachFile = Zotero.Promise.coroutine(function* (item, path) {
    if (!item.isRegularItem())
      throw "Zotero.ZotFile.attachFile(): 'item' is not a regular zotero attachment item.";
    let options = {
      file: path,
    ...
    // create linked attachment
      ...
      let filename = this.getFilename(item, OS.Path.basename(path)),
        subfolder =
          !this.getPref("import") & this.getPref("subfolder")
            ? this.getPref("subfolderFormat")
            : "",
  ...
  this.attachFileFromSourceDirectory = Zotero.Promise.coroutine(function* () {
    ...
    if (this.getPref("confirmation"))
      if (
        !confirm(
          `Do you want to rename and attach/link the file '${OS.Path.basename(
            paths[0],
          )}' to the currently selected Zotero item?`,
        )
      )
        return;
    // attach files
    var progress_win = this.progressWindow("ZotFile: New attachment(s) added");
   ...
   */
  this.renameAttachment = Zotero.Promise.coroutine(
    function* (att, imported, rename, folder, subfolder, verbose) {
      console.log("renameAttachment is called");
      // default arguments
      imported =
        typeof imported !== "undefined" ? imported : this.getPref("import");
      var subfolder_default =
        !imported & this.getPref("subfolder")
          ? this.getPref("subfolderFormat")
          : "";
      rename = typeof rename !== "undefined" ? rename : true;
      folder =
        typeof folder !== "undefined" ? folder : this.getPref("dest_dir");
      subfolder =
        typeof subfolder !== "undefined" ? subfolder : subfolder_default;
      verbose = typeof verbose !== "undefined" ? verbose : true;
      // check function arguments
      if (!att.isAttachment())
        throw "Zotero.ZotFile.renameAttachment(): No attachment item.";
      if (att.isTopLevelItem())
        throw "Zotero.ZotFile.renameAttachment(): Attachment is top-level item.";
      // set variables
      var win = Services.wm.getMostRecentWindow("navigator:browser"),
      ...
      if (!path)
        throw "Zotero.ZotFile.renameAttachment(): Attachment file does not exists.";
      ...
      if (imported && linkmode == Zotero.Attachments.LINK_MODE_LINKED_FILE) {
        console.log("(a) linked to imported attachment");
        ...
        for (let note of notes) {
          ...
          content = content.replace(
            new RegExp("open-pdf/([\\w\\W\\d]{1,10})_" + att.key, "g"),
            "open-pdf/$1_" + attNew.key,
          );
        ...
        // notification
        if (verbose)
          this.messages_report.push(
            this.ZFgetString("renaming.imported", [filename]),
          );
        return attNew;
      }
      // (b) imported to imported attachment (or cloud library)
      if (
        (imported && att.isImportedAttachment()) ||
        item.library.libraryType != "user"
      ) {
        console.log("(b) imported to imported attachment (or cloud library)");
        // rename file associated with attachment
      ...
      if (
        ...
        // move pdf file
        console.log("moveFile", path, location, filename);
        ...
        if (verbose)
          this.messages_report.push(
            this.ZFgetString("renaming.linked", [filename]),
          );
        return attNew;
      }
