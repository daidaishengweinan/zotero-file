...
Zotero.ZotFile = new function () {
    ...
    this.attachFileFromSourceDirectory = Zotero.Promise.coroutine(function* () {
        ...
        if (this.getPref("confirmation"))
            if (!confirm(`Do you want to rename and attach/link the file '${OS.Path.basename(paths[0])}' to the currently selected Zotero item?`))
                return;
        // attach files
    ...
    this.renameAttachment = Zotero.Promise.coroutine(function* (att, imported, rename, folder, subfolder, verbose) {
        ...
        if (att.isImportedAttachment() && !imported && item.library.libraryType == 'user') {
            console.log("(c) imported to linked attachment (only if library is local)")
            // move pdf file
        ...
        if (!att.isImportedAttachment() && !imported && item.library.libraryType == 'user') {
            console.log("(d) linked to linked attachment (only if library is local)")
            // relink attachment
