  ...
  getOutline: function (args) {
    ...
    OS.File.read(args.url).then(
      function onSuccess(array) {
        ...
        // get pdf outline
        PDFJS.getDocument(int8View).then(
          function (pdf) {
            ...
            pdf.getOutline().then(function (outline) {
              ...
              var buildIndex = function (obj) {
                if (obj.dest && obj.dest[0]) pageIndex.push(obj.dest);
              ...
              pdf.getDestinations().then(function (dest) {
                ...
                pageIndex
                  .reduce(function (sequence, ref) {
                    // Add these actions to the end of the sequence
                    return sequence
                      .then(function () {
                        // return pdf.getPageIndex(ref);
                        return typeof ref == "string"
                          ? ref in dest && dest[ref] !== null
                            ? pdf.getPageIndex(dest[ref][0])
                            : 0
                          : pdf.getPageIndex(ref[0]);
                      })
                      .then(function (page) {
                        var key =
                          typeof ref == "string" ? ref : JSON.stringify(ref[0]);
                        pageMap[key] = page;
                      });
                  }, Promise.resolve())
                  .then(function () {
