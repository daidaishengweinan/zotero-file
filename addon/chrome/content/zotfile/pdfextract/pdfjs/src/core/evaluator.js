  ...
  PartialEvaluator.prototype = {
      ...
      if (group) {
        ...
        if (isName(groupSubtype) && groupSubtype.name === "Transparency") {
          ...
          colorSpace = group.has("CS")
            ? ColorSpace.parse(group.get("CS"), this.xref, resources)
            : null;
        }
    ...
    buildPaintImageXObject: function PartialEvaluator_buildPaintImageXObject(
      ...
      PDFImage.buildImage(self.handler, self.xref, resources, image, inline)
        .then(function (imageObj) {
          ...
          self.handler.send(
            "obj",
            [objId, self.pageIndex, "Image", imgData],
            [imgData.data.buffer],
          );
        })
    ...
    handleSetFont: function PartialEvaluator_handleSetFont(
      ...
      return this.loadFont(fontName, fontRef, this.xref, resources)
        .then(function (translated) {
          ...
          return translated
            .loadType3Data(self, resources, operatorList)
            .then(function () {
              return translated;
            });
        })
    ...
    handleText: function PartialEvaluator_handleText(chars, state) {
      ...
      for (var i = 0; i < glyphs.length; i++) {
        if (glyphs[i] !== null) glyphs[i].str = glyphToUnicode(glyphs[i]);
      }
      var isAddToPathSet = !!(
    ...
    loadFont: function PartialEvaluator_loadFont(
      fontName,
      font,
      xref,
      resources,
      ...
      font.loadedName =
        "g_font_" + (fontRefIsDict ? fontName.replace(/\W/g, "") : fontID);
    ...
    handleColorN: function PartialEvaluator_handleColorN(
      operatorList,
      fn,
      args,
      cs,
    ...
    getOperatorList: function PartialEvaluator_getOperatorList(
      stream,
      resources,
      operatorList,
      initialState,
        ...
        while (!(stop = timeSlotManager.check())) {
          ...
          switch (fn | 0) {
            ...
            case OPS.setFillColorSpace:
              stateManager.state.fillColorSpace = ColorSpace.parse(
                args[0],
                xref,
                resources,
              );
            ...
            case OPS.setStrokeColorSpace:
              stateManager.state.strokeColorSpace = ColorSpace.parse(
                args[0],
                xref,
                resources,
              );
            ...
            case OPS.setGState:
              ...
              return self
                .setGState(resources, gState, operatorList, xref, stateManager)
                .then(function () {
                  next(resolve, reject);
                }, reject);
            case OPS.moveTo:
    ...
    getTextContent: function PartialEvaluator_getTextContent(
      ...
      function handleSetFont(fontName, fontRef) {
        return self
          .loadFont(fontName, fontRef, xref, resources)
          .then(function (translated) {
      ...
      function buildTextGeometry(chars, textChunk) {
        ...
        if (!textChunk.transform) {
          ...
          var trm = (textChunk.transform = Util.transform(
            textState.ctm,
        ...
        for (var i = 0; i < glyphs.length; i++) {
          ...
          if (!font.vertical) {
            ...
            tx =
              (w0 * textState.fontSize + textState.charSpacing) *
              ...
              for (var j = 0, jj = items.length; j < jj; j++) {
                if (typeof items[j] === "string") {
                  ...
                  if (!textState.font.vertical) {
                    ...
                    offset =
                      -val * textState.fontSize * textState.textMatrix[3];
    ...
    readToUnicode: function PartialEvaluator_readToUnicode(toUnicode) {
      var cmap,
        cmapObj = toUnicode;
      if (isName(cmapObj)) {
        cmap = CMapFactory.create(
          cmapObj,
          { url: PDFJS.cMapUrl, packed: PDFJS.cMapPacked },
          null,
        ).getMap();
    ...
    translateFont: function PartialEvaluator_translateFont(
      ...
      if (composite) {
        ...
        properties.cMap = CMapFactory.create(
          cidEncoding,
          { url: PDFJS.cMapUrl, packed: PDFJS.cMapPacked },
          null,
        );
      ...
      for (var i = 0, n = charProcKeys.length; i < n; ++i) {
        loadCharProcsPromise = loadCharProcsPromise.then(
          function (key) {
            ...
            return evaluator
              .getOperatorList(glyphStream, fontResources, operatorList)
              .then(
                function () {
    ...
    addOp: function (fn, args) {
      ...
      if (this.messageHandler) {
        if (this.fnArray.length >= CHUNK_SIZE) {
          ...
          this.fnArray.length >= CHUNK_SIZE_ABOUT &&
  ...
  addState(
    ...
    function (context) {
      ...
      while (i + 3 < ii) {
        ...
        if (
          argsArray[i + 1][0] !== firstTransformArg0 ||
          argsArray[i + 1][1] !== 0 ||
          argsArray[i + 1][2] !== 0 ||
  ...
  addState(
    ...
    function (context) {
      ...
      while (i + 4 < ii) {
        ...
        if (
          argsArray[i + 1][0] !== firstSetFontArg0 ||
