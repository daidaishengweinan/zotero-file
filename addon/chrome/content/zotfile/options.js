...
var disablePreference = function (setting, pref, revert) {
  ...
  if (typeof pref == "string")
    document.getElementById("id-zotfile-" + pref).disabled = !setting;
  if (typeof pref == "object")
    for (var i = 0; i < pref.length; i++)
      document.getElementById("id-zotfile-" + pref[i]).disabled = !setting;
  return setting;
}.bind(Zotero.ZotFile);
...
var updatePreferenceWindow = function (which) {
  ...
  if (Zotero.version[0] <= 2)
    document.getElementById("id-zotfile-removeDiacritics").disabled = true;

  // dis- and enable poppler extractor option
  document.getElementById("id-zotfile-pdfExtraction-UsePDFJS-false").disabled =
    !this.pdfAnnotations.popplerExtractorTool;

  var revert = which != "all" ? true : false;

  // Add 'et al'
  if (which == "etal" || which == "all")
    disablePreference("add_etal", "etal", revert);

  // max title length
  if (which == "truncate_title_max" || which == "all")
    disablePreference("truncate_title_max", "max_titlelength", revert);

  // max authors
  if (which == "truncate_authors" || which == "all")
    disablePreference(
      "truncate_authors",
      "max_authors",
      "number_truncate_authors",
      revert,
    );
  ...
  if (which == "subfolder" || which == "all")
    disablePreference("subfolder", "subfolderFormat", revert);

  // subfolder-tablet
  if (which == "subfolder-tablet" || which == "all")
    disablePreference("tablet-subfolder", "tablet-subfolderFormat", revert);

  // storeCopyOfFile-tablet
  if (which == "storeCopyOfFile" || which == "all")
    disablePreference(
      "tablet-storeCopyOfFile",
      "tablet-storeCopyOfFile_suffix",
      revert,
    );

  // batch renaming
  if (which == "confirm" || which == "all")
    disablePreference("confirmation_batch_ask", "confirmation_batch", revert);

  // filetypes
  if (which == "filetypes" || which == "all")
    disablePreference("useFileTypes", "filetypes", revert);

  // Source Folder
  if (which == "source" || which == "all") {
    disablePreference(
      "source_dir_ff",
      ["source_dir", "source_dir-button"],
      !revert,
    );
  ...
  if (which == "dest" || which == "all") {
    setting = disablePreference(
      "import",
      ["dest_dir", "dest_dir-button", "subfolder", "subfolderFormat"],
      !revert,
    );
    if (setting)
      document.getElementById("id-zotfile-subfolderFormat").disabled =
        !document.getElementById("pref-zotfile-subfolder").value;
    if (which != "all") updateFolderIcon("dest", true);
  }

  // userinput
  if (which == "userinput" || which == "all")
    disablePreference("userInput", "userInput_Default", revert);
  ...
  if (which == "zotfile-tablet" || which == "all") {
    setting = disablePreference(
      ...
      [
        "tablet-baseFolder",
        "tablet-dest_dir",
        "tablet-dest_dir-button",
        "tablet-dest_dir-show",
        "tablet-dest_dir-des",
        "tablet-label-subfolders",
        "tablet-projectFolders",
        "tablet-projectFolders-button",
        "tablet-subfolder",
        "tablet-subfolderFormat",
        "tablet-rename",
        "tablet-storeCopyOfFile",
        "tablet-storeCopyOfFile_suffix",
        "pdfExtraction-Pull",
    ...
    if (which == "zotfile-tablet") {
      if (!document.getElementById("pref-zotfile-tablet").value) {
        if (
          confirm(
            this.ZFgetString("tablet.createSavedSearches", [
              this.getPref("tablet.tag"),
            ]),
          )
        )
...
var checkRenameFormat = function (which) {
  try {
    ...
    if (item.isRegularItem())
      filename = this.getFilename(item, "", rename_format);
...
var updateFolderIcon = Zotero.Promise.coroutine(
  function* (which, revert) {
...
var checkFolderLocation = Zotero.Promise.coroutine(
  function* (folder) {
...
var previewFilename = function () {
  try {
    ...
    var item = items[0];
    // get renaming rules
    var rename_format = document.getElementById(
      "pref-zotfile-renameFormat",
    ).value;
...
var changedBasefolder = Zotero.Promise.coroutine(
  function* (dest_dir) {
    ...
    if (baseFolderOld != baseFolder && baseFolderOldValid) {
      ...
      if (baseFolderValid && atts.length > 0) {
        ...
        if (
          !confirm(
            this.ZFgetString("tablet.baseFolderChanged.prompt", [atts.length]),
          )
        )
          this.setPref("tablet.dest_dir", baseFolderOld);
      }
...
var editSubfolderSetting = function (index) {
  var treechildren = document.getElementById(
    "id-zotfile-tablet-projectFolders-rows",
  );
  var treerow;
  if (index != undefined || treechildren.childNodes.length < this.projectMax) {
    ...
    if (index != undefined) {
      if (index == "rightclickmenu") {
        var tree = document.getElementById(
          "id-zotfile-tablet-projectFolders-tree",
        );
    ...
    if (io.folder.substr(0, 1) != folder_sep)
      io.folder = folder_sep + io.folder;
    ...
    if (index != undefined) {
      ...
      if (folder.toLowerCase() != io.folder.toLowerCase())
        changedSubfolder(folder, io.folder);
    }
    ...
    this.infoWindow(
      ...
      this.ZFgetString("tablet.maxSubfolders", [this.projectMax]),
      8000,
    );
  }
}.bind(Zotero.ZotFile);
...
var buildFolderList = function () {
  ...
  var label = subfolders.map(function (obj) {
    return obj.label;
  });
  var folder = subfolders.map(function (obj) {
    return obj.path;
  });
...
var updateSubfolderPreferences = function () {
  ...
  for (var i = 0; i < treechildren.childNodes.length; i++) {
    ...
    if (label != "" && folder != "")
      subfolders.push({ label: label, path: folder });
  }
  ...
  function* (subfolder) {
    ...
    if (attInFolder.length > 0) {
      ...
      var userInput = this.promptUser(
        this.ZFgetString("tablet.attsInDeletedSub", [attInFolder.length]),
...
var changedSubfolder = Zotero.Promise.coroutine(
  function* (projectFolderOld, projectFolderNew) {
    // get attachments in old subfolder
    var attInFolder = yield this.Tablet.getAttachmentsOnTablet(
      projectFolderOld,
    );
    ...
    if (attInFolder.length > 0)
      confirmed = confirm(
        this.ZFgetString("tablet.moveAttsToNewSubfolder", [
          attInFolder.length,
          projectFolderOld,
          projectFolderNew,
        ]),
...
var updatePDFToolsStatus = Zotero.Promise.coroutine(
  function* () {
    ...
    var updateButton = document.getElementById(
      "pdf-annotations-extractor-update-button",
    );
    ...
    if (toolIsCompatible && toolIsRegistered) {
      ...
      // set version text
      var versionText = document.getElementById(
        "pdf-annotations-extractor-version",
      );
      versionText.setAttribute("hidden", false);
      versionText.setAttribute(
        "value",
        this.ZFgetString("extraction.popplerInstalledVersion", [
          installedVersion,
        ]),
      );
    }
...
var checkForUpdates = function (button, installedVersion) {
  var url =
    this.popplerExtractorBaseURL + this.popplerExtractorFileName + ".version";
...
var getInstalledVersion = Zotero.Promise.coroutine(
  function* () {
    ...
    d.start().then(
      function success(p) {
        ...
        var args = [
          ...
          "-q",
          file.path,
          "-d" + zotero_dir + "/ExtractPDFAnnotations",
        ];
        // run process
        if (!Zotero.isFx36) {
        ...
        var extractorFile = Zotero.File.pathToFile(
          Zotero.ZotFile.pdfAnnotations.popplerExtractorPath,
        );
        if (Zotero.isMac) {
        ...
        document.getElementById(
          "id-zotfile-pdfExtraction-UsePDFJS-false",
        ).disabled = false;
        // update settings
        updatePDFToolsStatus();
      },
      function (e) {
        ...
        Zotero.ZotFile.infoWindow(
          Zotero.ZotFile.ZFgetString("general.error"),
          "Unable to download poppler.",
          8000,
        );
      },
    );
